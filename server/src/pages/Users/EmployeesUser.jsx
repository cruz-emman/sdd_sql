...
const EmployeesUser = () => {
  ...
  useEffect(() =>{
    ...
    const getEmployeesUser = async () =>{
        try {
            const res = await usersRequest.get('/')
            let filteredUser = res.data.filter((item) => item.user_category === 'employees')
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link Link to={`/${pathname}/edit/${params.row.user_id}`}>
    ...
    <Box sx={{display:'flex', flexDirection:'row', gap:1}}>
        ...
        <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
          ...
          <Box sx={{display:'flex', alignItems:'center', justifyContent:'space-between', width: '100%'}}> 
            <Typography  variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Employees Users</Typography>
