...
const mysql2 = require('mysql2/promise');
 ...
 const addChoice = async (req,res) =>{
  ...
  const q = "INSERT INTO studentschoices (`question_id`, `question_choices`, `toggle`, `part`, `question_name`) VALUES (?,?,?,?,?)"
  try {
 ...
 const getChoices = async (req,res) =>{
    ...
    const q = "SELECT * FROM studentschoices WHERE question_id = ? && part = ?";
...
const getAllChoices = async (req,res) =>{
  const q = "SELECT * FROM studentschoices"
  try {
    const conn = await mysql2.createConnection({
      host: 'localhost',
      user: 'root',
 ...
 const deleteChoices = async (req,res) =>{
    const id = req.params.id;
    const q = "DELETE FROM studentschoices WHERE `idstudentschoices` = ?  " 
    try {
      const conn = await mysql2.createConnection({
        host: 'localhost',
        user: 'root',
  ...
  const getChoicesForUI = async(req,res) =>{
    ...
    const q = "SELECT * FROM studentschoices WHERE `question_name` = ? AND `part` = ?";
